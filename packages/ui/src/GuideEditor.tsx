...
export default function GuideEditor({
  ...
  useEffect(() => {
    if (!filePath) return
    adapter.cs2?.watchFile(filePath)
    const cleanup = adapter.cs2?.onFileChanged(async (changedPath) => {
  ...
  const handleRunInCS2 = async (command: string) => {
    ...
    try {
      ...
      const writeCfg = adapter.cs2?.writeCommand
      if (typeof writeCfg === 'function') {
        ...
        const send = adapter.cs2?.sendConsoleCommand
    ...
    <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
      ...
      <div className="flex items-center gap-1.5 flex-wrap px-4 py-1.5 bg-zinc-800/40 border-b border-zinc-700/60 shrink-0">
        ...
        <button
          ...
          className={btnSecondary}
          onClick={async () => {
            const res = await adapter.launchCS2?.()
      ...
      <div className="px-4 py-1.5 bg-red-950/20 border-b border-red-800/30 shrink-0">
        <p className="m-0 text-[0.68rem] text-red-300/85">
          ⚠ Saving created annotations uses CS2 `annotation_save`. If annotations were cleared in-game first, this can overwrite the file with only the current in-memory nodes.
        </p>
      </div>
          ...
          {lastCfgPath && !isMessageError && (
            <span className="text-zinc-600 flex items-center gap-1.5">
              ...
              <button
                ...
                onClick={() => adapter.clipboard?.showInFolder?.(lastCfgPath)}
              >
                Open folder
              </button>
            </span>
        ...
        {filePath && (
          <span className="flex items-center gap-1.5 text-[0.65rem] text-zinc-700">
            ...
            <button
              ...
              onClick={() => adapter.clipboard?.showInFolder?.(filePath)}
            >
              Open folder
            </button>
          </span>
...
function NodeEditForm({
  ...
  const handleCopySetpos = async () => {
    ...
    try {
      const copyViaIpc = adapter.clipboard?.write
