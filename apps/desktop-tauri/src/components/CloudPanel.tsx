...
// Deviation from brief: rather than the brief's inline `invoke('read_text_file')`
// + `cloudApi.cloudPushGuide({ ..., content })` call (Step 5, substitution 4),
// this uses the shared `TauriAdapter` instance's `cloudPushGuide`/`cloudPullGuide`,
// which already wrap that same file-read internally (see TauriAdapter.ts). This
// keeps CloudPanel free of direct `invoke()` calls — the adaptation notes for
// this task call this the preferred route.
...
export default function CloudPanel({ guides, statuses, loading, onRefresh, onStatusChange }: Props) {
  ...
  useEffect(() => {
    ...
    const unsub = onAuthStateChanged(setAuthState)
    return unsub
  }, [])
  ...
  const handlePush = useCallback(async (guide: GuideSummary) => {
    ...
    try {
      const res = await adapter.cloudPushGuide!({
        filePath: guide.id,
        title: guide.name,
        map: guide.mapName ?? '',
        nodeCount: 0,
  ...
  const handlePull = useCallback(async (guide: GuideSummary) => {
    ...
    setMessages((prev) => { const n = { ...prev }; delete n[guide.id]; return n })
    try {
      const res = await adapter.cloudPullGuide!({ cloudId: state.cloudId, filePath: guide.id })
